...
export default function InvestmentPage() {
  ...
  const compareRows = useMemo(() => {
    ...
    const liqA = calcLiquidationPrice(a.entryPrice, a.leverage, direction)
    const liqB = calcLiquidationPrice(b.entryPrice, b.leverage, direction)
    return [
      ...
      { label: `청산가 (${direction === 'long' ? '롱' : '숏'})`, a: fmt(liqA), b: fmt(liqB), unit: '원' },
    ]
  }, [a, b, direction])
    ...
    <div className="max-w-3xl mx-auto space-y-4">
      ...
      {compareMode ? (
        <div className="space-y-3">
          ...
          <CompareView labelA="A" labelB="B" rows={compareRows} />
        </div>
      ) : (
        <InvestmentCalc />
      )}
